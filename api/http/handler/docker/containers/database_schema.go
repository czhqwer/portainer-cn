...
func (handler *Handler) databaseConnectionSchema(w http.ResponseWriter, r *http.Request) *httperror.HandlerError {
	...
	result, err := executeContainerDatabaseSchema(r.Context(), client, containerID, *connection)
	if err != nil {
...
func executeContainerDatabaseSchema(ctx context.Context, client *dockerclient.Client, containerID string, connection portainer.DatabaseConnection) (*databaseSchemaResponse, error) {
	switch connection.Type {
	case "mysql", "mariadb":
		return mysqlDatabaseSchema(ctx, client, containerID, connection)
	case "postgres":
		return postgresDatabaseSchema(ctx, client, containerID, connection)
	case "redis":
...
func mysqlDatabaseSchema(ctx context.Context, client *dockerclient.Client, containerID string, connection portainer.DatabaseConnection) (*databaseSchemaResponse, error) {
	if connection.Database != "" {
		tables, err := mysqlTables(ctx, client, containerID, connection, connection.Database)
		if err != nil {
			return nil, err
		}
	...
	result, err := executeDatabaseQuery(ctx, client, containerID, connection, "SHOW DATABASES", false)
	if err != nil {
		return nil, err
	}
	...
	for _, row := range result.Rows {
		...
		tables, err := mysqlTables(ctx, client, containerID, connection, name)
		if err != nil {
			return nil, err
		}
...
func mysqlTables(ctx context.Context, client *dockerclient.Client, containerID string, connection portainer.DatabaseConnection, database string) ([]databaseSchemaTable, error) {
	query := fmt.Sprintf("SHOW FULL TABLES FROM `%s`", strings.ReplaceAll(database, "`", "``"))
	result, err := executeDatabaseQuery(ctx, client, containerID, connection, query, false)
	if err != nil {
		return nil, err
	}
...
func postgresDatabaseSchema(ctx context.Context, client *dockerclient.Client, containerID string, connection portainer.DatabaseConnection) (*databaseSchemaResponse, error) {
	query := `
