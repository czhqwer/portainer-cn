...
func TestPlatformJavaBuildUsesControlledTemplateAndCleansFailure(t *testing.T) {
	...
	require.NotEmpty(t, result.TaskEvents)
	...
	require.Equal(t, "complete", persisted.TaskEvents[len(persisted.TaskEvents)-1].Stage)
	require.Equal(t, artifactTaskEventFailed, persisted.TaskEvents[len(persisted.TaskEvents)-1].Status)
	require.Equal(t, "CONTROLLED_BUILD_FAILED", persisted.TaskEvents[len(persisted.TaskEvents)-1].Reason)
